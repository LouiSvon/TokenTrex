...
- States defined in `TrexState` enum in `TrexSpriteRenderer.swift` — keep switch cases and sprite variables in the same order
- Build & run: `pkill -f TokenTrex; ./build.sh && open build/TokenTrex.app`
...
6. Replace the corresponding `*Frames` variable in `TrexSpriteRenderer.swift`
...
1. **Bump version** in `TokenTrex/Info.plist` (`CFBundleShortVersionString` + `CFBundleVersion`) and commit
2. Create an **annotated tag**: `git tag -a vX.Y.Z -m "vX.Y.Z"`
3. **Push** commits and tag: `git push origin main --tags`
4. **Clean build**: `rm -rf .build build && ./build.sh` (cached builds can produce the same binary, causing macOS to silently deny Keychain access)
5. **Verify**: open `build/TokenTrex.app` manually and confirm Keychain prompt appears and usage data loads
6. Create DMG: `hdiutil create -volname TokenTrex -srcfolder build/TokenTrex.app -ov -format UDZO build/TokenTrex.dmg`
7. Create GitHub release **with DMG attached in one step**: `gh release create vX.Y.Z build/TokenTrex.dmg`
