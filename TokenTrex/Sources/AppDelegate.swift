...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var currentState: TrexState = .idle
    ...
    @objc private func showMenu() {
        ...
        // Center the menu horizontally under the icon, with a small gap below the menu bar
    ...
    private func observeUsageChanges() {
        ...
        usageManager.$usagePercent
            ...
            .sink { [weak self] _, _ in
                ...
                let newState = self.usageManager.trexState
    ...
    private func switchToState(_ state: TrexState) {
        currentState = state
        currentFrames = TrexSpriteRenderer.frames(for: state)
        currentFrameIndex = 0
    ...
    /// Composites percentage text into the sprite image.
    private func updateStatusImage() {
        guard !currentFrames.isEmpty else { return }
        let trexFrame = currentFrames[currentFrameIndex]

        if usageManager.showPercentageInMenuBar {
            statusItem.button?.image = compositeImage(trexFrame: trexFrame, text: "\(Int(usageManager.usagePercent))%")
        } else {
            statusItem.button?.image = trexFrame
        }
    }

    private func compositeImage(trexFrame: NSImage, text: String) -> NSImage {
        ...
        let totalWidth = textSize.width + spacing + trexFrame.size.width
        let height = max(trexFrame.size.height, textSize.height)
        ...
        trexFrame.draw(
            in: NSRect(x: textSize.width + spacing, y: (height - trexFrame.size.height) / 2,
                       width: trexFrame.size.width, height: trexFrame.size.height),
    ...
    private func animationInterval(for state: TrexState) -> TimeInterval {
        switch state {
